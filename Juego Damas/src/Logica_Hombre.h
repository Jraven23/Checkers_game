...
class Logica_Hombre:public Logica
{
protected:
...
public:
	...
	Logica_Hombre(Piece::type_t type):type(type),pos(-1,-1), mejor_opcion_dcha(Piece::NADA), mejor_opcion_izq(Piece::NADA) {}
	Logica_Hombre(const Logica_Hombre& logica):type(logica.type),mejor_opcion_dcha(logica.mejor_opcion_dcha), mejor_opcion_izq(logica.mejor_opcion_izq),mejor_dcha(logica.mejor_dcha), mejor_izq(logica.mejor_izq) {}
	...
	int mueve(Piece** board, int N, Vector2D presente, Vector2D futuro);
